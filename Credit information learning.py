...
class SoftmaxWithLoss:
    def forward(self, A, y=None):
        ...
        if y is None:
            return self.probs  # for prediction
        self.y = y
        m = y.shape[0]
...
class ScratchDeepNeuralNetworkClassifier:
    def __init__(self, n_features, layer_sizes, activations,
                 initializer="xavier", optimizer="sgd", lr=0.01, sigma=0.01,
                 batch_size=64, epochs=10, verbose=True, seed=42):
        np.random.seed(seed)
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        for epoch in range(1, self.epochs + 1):
            ...
            for i in range(0, X.shape[0], self.batch_size):
                ...
                steps += 1
            if self.verbose:
                msg = f"Epoch {epoch}/{self.epochs} - loss: {epoch_loss/steps:.4f}"
    ...
    def predict(self, X):
        ...
        for fc, act in zip(self.layers, self.activations):
            out = act.forward(fc.forward(out))
        probs = self.softmax.forward(out)  # reuse softmax
...
if __name__ == "__main__":
    ...
    model = ScratchDeepNeuralNetworkClassifier(
        ...
        activations=["relu", "relu", "identity"],  # last goes to softmax
        initializer="he",
        optimizer="adagrad",
        lr=0.05,
        batch_size=128,
